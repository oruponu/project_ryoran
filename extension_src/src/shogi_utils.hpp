...
// 盤上の駒の価値を調整
inline constexpr int apply_board_discount(int score) {
	return score - (score * 104 / 1024);
}
...
struct UndoInfo {
	...
	UndoInfo() : captured_type(static_cast<uint8_t>(PieceType::EMPTY)), captured_promoted(false), prev_hash(0), prev_score(0) {
		prev_pawn_cols[0] = 0;
		prev_pawn_cols[1] = 0;
	}
};
